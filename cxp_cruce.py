...
def _es_columna_saldo_mes(titulo: str) -> bool:
    norm = _normalizar(str(titulo or ""))
    if not norm or norm.startswith("estado actual"):
        return False
...
def _listar_columnas_corte_mes_cps(ws) -> list[tuple[int, int]]:
    """Alias: misma detección que Liquidados con saldo."""
    return _listar_columnas_saldo_mes(ws)


def _fill_encabezado_por_mes(mes_num: int) -> PatternFill:
    """Mes par → azul; mes impar → amarillo (todas las pestañas de una columna SALDO)."""
    return FILL_ENCABEZADO_AMARILLO if (mes_num % 2) else FILL_ENCABEZADO_AZUL


def _filas_encabezado_vertical(ws, fila_hdr: int | None = None) -> tuple[int, int]:
    """Fila(s) del título (p. ej. 1:2 combinadas en Liquidados; 3 sola en Cps)."""
    if fila_hdr is None:
        fila_hdr = _fila_encabezado_hoja_datos(ws)
    fila_fin = fila_hdr
    for rango in ws.merged_cells.ranges:
...
def _copiar_ancho_columna_hoja(ws, col_origen: int, col_destino: int) -> None:
...
def _agregar_columna_corte_en_hoja(ws, titulo: str, fecha: datetime | date) -> int:
    ...
    if col_prev:
        _copiar_estilo_columna_hoja(
            ws, col_prev, nueva_col, sin_relleno_desde_fila=fila_datos
        )
        _copiar_ancho_columna_hoja(ws, col_prev, nueva_col)
        for fila in (FILA_CONTEO_CONTRATOS, FILA_SUMA_CONTRATOS):
            _copiar_estilo_celda(ws.cell(fila, col_prev), ws.cell(fila, nueva_col))
    else:
        col_sf = _columna_estilo_saldo_final(ws)
        _copiar_estilo_columna_hoja(
            ws, col_sf, nueva_col, sin_relleno_desde_fila=fila_datos
        )
        for fila in (FILA_CONTEO_CONTRATOS, FILA_SUMA_CONTRATOS):
...
def _actualizar_resumen_filas_1_2(
    ws, col_corte: int, col_prev: int | None = None
) -> None:
    """
    ...
    col_estilo = col_prev or _columna_estilo_saldo_final(ws)
...
def exportar_contratos_preservando_formato(
    ...
    for fila, valor in valores_por_fila.items():
        ...
        if col_prev:
            _copiar_estilo_celda_sin_relleno(ws.cell(fila, col_prev), celda)
        else:
